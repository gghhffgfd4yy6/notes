...
- 变异**分数门禁**（此前没有任何分数门禁）：`stryker.config.js` 的 `thresholds.break` 由 `null` 改为 `65`（按**段**判定——`mutation.yml` 每段独立 `npx stryker run --mutate "<段>"`，`@stryker-mutator/core@10.0.0` 的 `determineExitCode` 在 `mutationScore < 65` 时以退出码 1 结束，故某段跌破 65 时该矩阵 job 红、其余段照跑、日报照发）。取值依据为实测基线（2026-09-18 日报：合计 81.27%，最低段 `message-store` 69.38%），阈值低于最低段 4.38 个百分点 ⇒ 按当前基线不会误红，而单段真实退化 4 个百分点以上即被拦下；且日报脚本的计分分母（`total`，含 RuntimeError/CompileError/Ignored/Pending）不小于 stryker 的 `totalValid`，同一份报告在 stryker 侧的分只会更高。`scripts/mutation-report.js` **有意**不加分数 throw（其 `main()` 先验证后发 Issue，会在最需要看分数时连日报一起吞掉），只保留既有的缺段/多余段/重复段/陈旧报告四类 `throw`（完整性真门禁，未放宽）。「只删/弱化测试」这条边界已在 PR #156 返工中收口（缓存 key 与 `restore-keys` 前缀纳入测试侧指纹，见下条），残余边界只剩分数门禁的固有性质：**弱化测试后新跑一轮的分数仍 ≥65 就照样绿**（它拦的是分数真的掉下来，不是测试被人改过）。
