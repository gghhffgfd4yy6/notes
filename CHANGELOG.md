...
- 未修复清单批次 B10：跨进程锁（`run.log` 与通道健康状态）的陈旧回收改为「mtime 超龄**且**持有进程已确认退出」（此前只看 mtime，被暂停 >10s 的存活进程锁会被第二个进程抢占，两个进程同时进入读改写临界区），锁内改写 `pid:starttime` 令牌供存活复核与 PID 复用判别；通道健康告警的网络 await 移出临界区（锁内只读改写、锁外发送），告警发送失败不再让其他轮次跳过健康更新。通道失败告警改为按「告警尝试」计时（此前只在发送成功后回填 `lastAlertAt`，告警通道不可用时 `intervalMs` 限频完全失效、每轮重发）；恢复告警改为确认送达后才清零（新增 `recoverAlertPending` 标记，此前发送失败即永久丢失恢复通知）。单条推送的内容渲染异常按单条推送失败处理（警告 + 不写缓存 + 下轮重试），不再中止整轮并跳过缓存写入。HTML 转 Markdown：属性值引号只在紧跟 `=` 时才算定界（此前未加引号的属性值里的撇号会被当成引号起点，吞掉其后所有 `<a>`/`<h1>`）。
...
- 依赖预检（`scripts/check-deps.js`）：探测清单改由 `package.json` 的 `dependencies`/`optionalDependencies` 派生（此前硬编码 `got`/`re2`，声明了却没装的运行时依赖永远发现不了）；判定改「先 resolve 后 load」两段——解析不到才是「缺少」，能解析但加载抛错（`ERR_REQUIRE_ESM`、内部依赖缺失、原生绑定损坏）归入「已安装但不可用」并按依赖给出对应修复指引，根因（含 `error.code`）进输出；补 Node 版本闸门（对照 `engines.node` 与 re2 自身更严的 `engines.node`，README 已列的 23.x/24.0–24.14/25.x 现在会被拦下，无法解析的范围写法只告警不误红）；补 `require.main` 守卫（`node scripts/check-deps.js` 此前只加载模块，恒 exit 0 且零输出）；默认 `load` 与 `resolve` 统一以项目根为基准（`scripts/` 下出现同名包不再可能「resolve 一个实例、require 另一个」）。
- 变异报告读取（`scripts/mutation-json.js`）：整文件读入前先 `statSync` 预检大小与文件类型——此前唯一的尺寸守卫落在 `Buffer.concat` 分配峰值之后，超限输入要先付一次整份报告的分配才发现放不下；非普通文件（目录/FIFO/socket）现在在读入前带路径拒绝（目录此前抛不带路径的 `EISDIR`，FIFO 会挂住进程）。

- DNS 与预热：`prewarmDns` 与真实请求终于共用同一缓存条目（key 收敛为 hostname+family，此前把 Node 传入的 `hints`/`all`/`verbatim` 编进 key，预热写进一个永不被读的条目，默认配置下 DNS 预热完全无效；底层统一按 `all:true` 解析、按各调用方 `all` 适配回调形状）；`ETIMEDOUT` 计入 DNS 失效码（重试窗口远短于 60s TTL，超时后不再反复复用同一失效地址），证书主机名不匹配（`ERR_TLS_CERT_ALTNAME_INVALID`）不再计入（它与「缓存地址已失效」无关，只会每次重试白解析一次）；预热结果补 `kind:'dns'|'tls'` 可按字段区分（此前两者都只带 hostname）；`got` 替身缺 `stream` 时 `prewarmTls` 报 `ok:false`（此前未建连却报成功）。
- 推送出口：`WX_pusher_channels` 多应用配置被丢弃（非法 JSON、数组项缺 `appToken`/`topicIds`、形状不是数组）时显式告警（此前静默回退旧字段，用户看到「推送成功」却少推其余应用）；`$.post`/`$.get` 补 20MB 流式响应体上限（复用 `xbk_http.DEFAULT_MAX_BODY`：官方 got 走流式限长读取，超限报 `EBODYLIMIT` 并销毁流，无 `stream` 的 got 替身仍走原 promise 路径）。
