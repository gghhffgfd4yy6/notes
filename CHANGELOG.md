...
## v3.169（文档去静态版本信息：README 不再维护版本号/测试数）

- **背景**：README/FILE_INDEX/REVIEW_DECISIONS 曾硬编码「当前版本 v3.xxx」「N 个测试全绿」「约 N 行」等静态信息——每次版本迭代都要手动同步且易漏（如文件头注释测试数曾滞后 736 vs 实际 761）
- **README**：删「当前版本 v3.168」「766 个测试全绿」「三套件分工数字」「目录结构测试数」→ 版本指向 package.json/CHANGELOG、测试数以 `node run_tests.js` 实际输出为准
- **FILE_INDEX**：删行数（约 1776/792/6210/1595/666 行）、测试数（约 643/86/37）、运行耗时、「文件头版本号需人工维护」说明
- **结构契约数字清除**（第二轮）：层数（9 层→分层）、导出数（33 个导出键）、快照数（6 个完整输出）、通道数（9 个推送通道）、过滤字段数（11 个）、行号区间（9 层表格/Config 1-90 行等全部删除）、测试强度参数（2^10 组合/100 层 HTML/性能基准次数/28 实体）——代码结构变化无需再同步文档
- **REVIEW_DECISIONS**：当前状态节动态化（测试数量不维护具体数字，以运行结果为准）；历史记录（v3.79 四方等、阶段4 评估 1357 行/33 导出）保留为历史事实
- **101 章版本一致性**：四方 → 三方（文件头 ↔ CHANGELOG 最新 ↔ package.json）；删 README 校验测试（README 不再含版本号）；文件头注释删除「N全绿(…)」测试数（版本号保留，仍由 101 章自动校验）
