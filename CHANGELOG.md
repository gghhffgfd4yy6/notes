...
- **修复**：`{Html内容}` 路径统一先解码实体，再清理主动标签、事件属性、危险 URL、xlink/formaction/srcset/style 等加载路径；普通安全 HTML 保持可用。
- **回归测试**：覆盖 script、iframe、svg、onerror、xlink、srcset、style 和实体编码主动标签；删除新清理链路的变异会被测试捕获。
