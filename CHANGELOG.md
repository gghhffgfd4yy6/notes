...
## v3.269（P1 故障韧性三连修·PR #20，2026-08-23）

- **R1 本地留痕**：告警发送失败时将摘要、时间戳、版本与单行化原因写入 `run.log`，避免告警静默丢失。
- **R2 递增退避**：连续可重试失败采用 1 秒至 300 秒的递增退避，不因临时故障直接退出；永久错误或配置错误仍退出。
- **R3 即时重试**：`pushOne` 对失败通道进行短退避重试，成功通道不受阻塞，重试失败由去重保护并在下一轮补推。
- **评审修复**：修正 `package.json` 路径，并规整告警原因中的换行，保持日志一行一条。
- **验证**：lint、版本一致性检查及完整测试套件。
