...
- **后续验证（v3.218/3.219）**：
  ...
  - WxPusher TLS 并行预取（3 连接）——与线报接口请求重叠建连；预取**不 await**（后台并行，推送不等预取，推送结束后才收尾），避免"等预取 10 个连接建完"的固定延迟（实测 await 预取 10 连接：预处理 0.9s、总计 1.53s；后台预取 3 连接：预处理 0.04s、总计 0.75s）。预取连接被推送复用时 tls=0；预取失败不阻断主流程。
