...
## 24. none 模式对"原始含 bug 提交"的检出率验证（2026-08-09）

- **目的**：用户要求验证 none 模式能否独立审出"bug 还存在时"的问题（非修复后）。
- **结果**（5 个已知 bug 的原始提交，none 完整 review）：

| 提交 | 已知 bug | none 审出 |
|---|---|---|
| 18650cb | 预热 P3×2 | ✅ |
| 1163a11 | 空 env P2 | ✅ |
| ef1116e | getNotify P2 | ✅ |
| b22f7d6 | 缓存恢复 P3 | ✅ |
| d3e7923 | 企业微信分类 P2 | ❌ 漏报 |

- **结论**：none 检出率 **4/5（80%）**；1 条漏报（d3e7923 企业微信——none 注意力分配在 permanentMessage 正则点，该点经查是项目测试锁定的设计语义）。
- **策略修正**：none 适合快速初筛（15s/个）；**关键/高风险提交（大 diff、分类/推送核心）保留 low 复核**（74s，可补漏）。两种模式盲区互补。
- **附带发现**：none 报的 permanentMessage 优先于 5xx 检查是设计冲突（d3e7923 测试明确 'invalid token'+500→permanent，消息语义优先）——不采纳 AI 建议；1163a11 的 ensureDependencies 只查 got 是合理设计（项目唯一 npm 依赖）。

## 2026-08-10 · v3-deep-audit 第三轮（d51bf29 之后）

**背景**：Codex 并行子代理审查（安全/架构/质量/链路 4 视角）。

| 提交 | 发现 | 处置 |
|---|---|---|
| d51bf29 | Fix4 await 修复 PASS（10s 阻塞为 v3.164 刻意取舍）| ✅ 无需动作 |
| 8611206 | 截断丢失通道：方案A（持久化队列 50-100行）可行 | 🟡 暂缓 |
| 8611206 | 无运行互斥锁：但 runLoop 顺序执行（await run 完成后下轮），常驻不重叠 | ⚪ 已知取舍 |
| 8611206 | 架构拆分：影响面大（测试 require/延迟加载/引用全动）| ⚪ 已知取舍 |

- **截断方案 A 暂缓理由**：触发条件太少（正常 ~20 条 << maxPerRun 100，仅接口异常时才截断）；本轮先改告警文案（已丢弃→已暂存待补推，v3.241）避免误导；持久化队列留待异常场景频发时再上。
- **互斥锁降级**：xbk_loop.js runLoop 为 `while { await run() }` 顺序执行，进程内永不重叠；仅外部 cron 多进程并行才可能触发，属部署层面约束。
- **子代理审查价值验证**：await 回归（8611206 的 fire-and-forget）就是子代理抓到的——多视角交叉审查能发现"自己改自己"的盲区。

## 2026-08-15 · 手机端 AI 只读审查 P3 处置（PR #20 之后）

**背景**：手机端 AI 6 份只读审查报告共 24 条 P3（风格/可读性/加固），A 类 12 条低成本高价值已随 PR 修复（文案 3、防御加固 4、性能优化 3、注释锁定 2）；以下 12 条评估后**记录不修**（修复风险 > 收益），供后人知悉取舍理由。

| 位置 | 发现 | 不修理由 |
|---|---|---|
| run L3138-L3145 | 预热连接数与实际推送并发窗口不对齐（注释与实现口径不一致） | 仅注释口径问题，预热为尽力而为优化，改行为有回归风险 |
| run L3555 | 顺序模式最后一条推送后仍等待 pushInterval | 收尾延迟为毫秒级，改时序可能影响既有测试与运行节奏 |
| run L3312-L3314 | 全空消息静默跳过且不计数 | 系统级统一行为（saveBatch/_upsert/has 同口径），改统计需同步多处口径，属设计选择 |
| MessageStore L2428/L2540 | saveBatch 返回类型不一致（undefined vs boolean）且失败不上抛 | API 变更牵涉 4 个调用方 + 测试，当前失败有 warn 日志可感知，收益 < 风险 |
| MessageStore L2428 | 非数组 newMessages 静默返回无日志 | 入口已有 Array.isArray 守卫，调用方均传数组，补日志收益极低 |
| MessageStore L2256/L2260/L2294/L2286 | try{delete}catch / try{set.add}catch 永不抛错路径的防御噪音 | 纯删空 catch 收益低，且删除后若环境异常失去兜底 |
| 路径安全 L1991-L2023 | cacheDir getter 每次调用 realpathSync（热路径重复校验） | 缓存校验结果需按 mtime/inode 失效，复杂度 > 收益；注释已说明权衡 |
| 过滤 L971-980 | pingbitime 多行含 ### 一律 timeActive=true（全 0 也激活）；简单形式无上限判定 | compileRules 已有上限处理（PINGBITIME_MAX_DAYS），过度失效安全侧无害 |
| 过滤 L1956 vs L1958 | if(!item) return false 与字段缺失放行不对称 | 防御性选择可理解（item 缺失 = 不匹配），统一放行反而引入语义变化 |
