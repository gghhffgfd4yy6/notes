//******** 线报酷推送脚本 v3.100 — 规则预编译 + 白名单重构 + HTML实体解码 + 原子写入 + 审查加固 + 日期解析统一 + 审查项批量 + 配置校验 + 运行日志增强 + 口径统一 + 推送模板/截断可配置 + 标题兜底截断 + 工程化 + 密钥示例 + domain校验 + 链接占位符安全 + 推送模块密钥泄露修复 + 失败日志统一 + got加固 + 版本四方一致 + 配置防御 + CLI兜底 + UA版本化 + 实体扩展 + 并行组合测试 + href换行剥离 + 一言防御 + runlog耗时 + 文档同步 + md链路锁定 + 测试隔离 + readme同步 + index同步 + domain斜杠防御 + 配置矩阵 + 目录同步 + 默认值契约 + 记录同步 + example补全 + 里程碑 ********
