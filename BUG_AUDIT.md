# V3 Bug、P1/P2 审计与验证记录

> 本文合并原 `BUG_HUNT.md` 与 `BUG_AUDIT_P1_P2.md`。
> 前半部分记录真实 Bug 的触发、修复和验证；后半部分记录 P1/P2 审计、资源生命周期和稳定性验证。
> 当前测试结果以 `npm test` 实际输出为准；历史版本号和历史测试数据仅保留为历史事实。
...
## v3.200：数值配置为 Symbol 导致主流程崩溃 ✅ 已修复

- **触发场景**：通过公开 `Config` 将 `api.timeout`、`push.maxPerRun` 等数值配置设为 `Symbol`，或设为 `valueOf()` 会抛异常的对象。
- **根因**：`Utils.num()` 直接执行 `Number(v)`，异常没有在统一数值配置入口被捕获；运行时告警又直接把原值插入模板，可能在错误处理路径再次触发 Symbol 转字符串异常。
- **影响**：主流程可能在配置校验阶段中断，无法继续拉取或推送，并进入异常告警路径。
...
## v3.202：配置告警脏值导致 App.run 崩溃 ✅ 已修复

- **触发场景**：`Config.cache.maxSize`、`Config.domain` 或 `Config.push.mode` 为 `Symbol` 等无法安全插值的脏值。
- **根因链路**：App.run 的配置校验发现非法值后，`console.warn` 模板直接插入原始配置；`Utils.num()`、domain getter 和模式判断虽有部分防御，但告警显示路径仍会因 Symbol 转字符串抛错。
- **影响**：本应“告警并回退/降级”的配置会升级为主流程失败，可能在拉取或推送前中断。
- **修复**：App.run 增加统一 `safeConfigText()`，覆盖缓存上限、域名和推送模式告警。
- **验证**：三类 Symbol 配置完整链路测试、并行集成测试和全量测试覆盖。

## v3.203：推送脱敏函数对异常对象崩溃 ✅ 已修复

- **触发场景**：公开调用 `maskKey()`、`maskUrl()`，或推送错误摘要中的 `message` 是 `toString()` 会抛异常的对象。
- **根因**：脱敏函数直接执行 `String(value)`；`safeErr()` 在处理异常 `message` 时会进入该路径，异常摘要可能发生二次抛错。
- **影响**：低概率脏响应或公开 API 调用可能中断错误处理/日志脱敏；不改变正常通道成功失败语义。
- **修复**：新增统一 `safeString()`，脱敏失败时返回空字符串并继续生成安全摘要。
- **验证**：异常对象回归测试、通道测试和全量测试覆盖。

## v3.204：缓存落盘失败污染内存缓存导致消息被吞 ✅ 已修复

- **触发场景**：同一进程多次调用 `App.run()` 或公开 `saveBatch()`，首次缓存原子写入因磁盘/权限/rename 失败。
- **根因链路**：`saveBatch()` 直接修改 `readMessages()` 返回的内存数组；`saveMessages()` 落盘失败后仍将新数组写入内存缓存。后续 `readMessages()` 优先返回内存缓存，导致未落盘消息被误判为已处理。
- **影响**：新进程通常会重新读取磁盘，但长生命周期调用方可能在恢复后仍跳过该消息，造成消息不重试。
- **修复**：`saveBatch()` 先复制内存缓存；`saveMessages()` 在序列化/写入/rename 失败时恢复旧内存快照，仅成功落盘后更新内存缓存。
- **验证**：rename 失败故障注入、单元测试、并行集成测试和全量测试覆盖。

## v3.205：状态持久化失败导致告警/日报重复，并补充磁盘余量监测 ✅ 已修复

- **触发场景**：告警或日报已发送成功，但 `alert.state`/`report.state` 因磁盘满、权限或 rename 失败无法写入。
- **根因链路**：发送 Promise 成功后状态写入结果被忽略；下一次运行仍读取旧状态，告警限频或日报日期无法推进。
- **修复**：保留进程内告警时间和日报状态；状态持久化失败时输出明确提示，不再误报“状态已保存”；通过 `fs.statfsSync` 监测缓存所在文件系统余量，低于阈值只告警不阻断。
- **兼容性**：`statfsSync` 不存在或调用失败时静默跳过；默认阈值可通过 `Config.storage.minFreeBytes` 调整。
- **验证**：低磁盘余量回归测试、全量测试覆盖。

## v3.206：非法八位零日期误识别为 Unix 时间戳 ✅ 已修复

- **触发场景**：接口 `posttime`/注册时间出现 `00000000` 等八位数字脏日期。
- **根因**：八位日期校验失败后继续进入时间戳判断；`Number('00000000') === 0`，被误判为 1970-01-01。
- **影响**：日期显示错误；注册时间进入天数过滤时可能被当成极老账号。
- **修复**：严格八位数字输入在非法 `YYYYMMDD` 校验后立即返回无效，不再进入 Unix 时间戳分支。
- **验证**：新增 `00000000` 日期回归测试。

## v3.226：常驻循环按失败类型有限重试/立即停止 ✅ 已修复

- **触发场景**：青龙常驻入口中，`App.run()` 连续失败或推送全部失败。
- **根因**：`runLoop()` 对所有异常统一调用 `onError` 后继续；而“推送全部失败”在 `App.run()` 中只返回摘要、不抛异常，因此常驻调度会把持续失败当作正常完成，进程保持运行且退出码为 0。
- **影响**：接口永久不可用、配置错误、权限错误或所有通道失效时，青龙调度器无法通过退出码识别任务已经失效；失败还可能在零间隔配置下快速空转。
- **修复**：新增统一失败分类；网络抖动、超时、限流和服务端暂时错误有限重试并退避；明确配置/权限/参数/地址/响应契约错误立即停止；运行摘要中的推送全部失败纳入同一分类；混合原因中只要仍有可重试因素就保留重试机会，仅剩永久失败时才停止；部分通道成功不熔断；性能预热失败单独记录，不计入业务失败；成功一轮清零连续失败状态。
- **验证**：回归测试覆盖永久错误一次停止、可重试错误达到阈值停止、部分成功不熔断、失败后恢复、推送全部失败摘要结构化和预热失败隔离。

## v3.227：常驻刷新任务挂起导致循环/停止阻塞 ✅ 已修复

- **触发场景**：DNS/TLS 性能刷新 Promise 长时间不完成，或底层解析任务挂起。
- **根因**：`runLoop()` 使用 `Promise.all([intervalTask, refreshTask])` 等待刷新任务，但刷新任务没有独立超时；即使 AbortSignal 已触发，未结束的刷新 Promise 仍会让循环无法返回。
- **影响**：常驻入口可能不进入下一轮，SIGTERM/SIGINT 也不能及时完成安全停止；性能优化任务反过来阻塞业务调度。
- **修复**：新增有界刷新包装，默认刷新超时后进入独立 `onIntervalError`，并支持 AbortSignal 取消；刷新超时不计入业务连续失败。
- **验证**：永久挂起刷新注入测试确认超时后循环可退出，正常全量回归通过。
...
## v3.230：V3 统一契约收敛 ✅ 已修复

本轮不按入口分别打补丁，而是围绕消息身份、URL、脏字段、脱敏、失败分类、传输参数、缓存事务和日期解析建立统一契约。

### 统一消息身份与 URL

- 新增统一身份函数：有效 ID → `id:`；无 ID 的有效 URL → `url:`；其余 → 稳定匿名合成键。
- App 批内判重、缓存索引、`_findDedupIndex`、`saveBatch`、截断未推排除和成功缓存写入统一复用身份语义。
- URL 只接受字符串；对象、数组、数字、Symbol 和历史伪值不再通过 `String()` 进入判重。
- `safeUrl` 统一处理危险协议、实体编码、控制字符、换行和最终补链路径。
- 旧缓存继续可读；伪 URL 不再参与有效 URL 判重，允许一次保守重推，不静默少推。

### 脏字段与日志

- 模板、推送日志、错误摘要和关键配置路径统一使用安全字符串化/安全字段读取。
- Symbol、异常 getter、循环对象和孤立代理不再因日志或模板格式化破坏推送成功后的缓存流程。
- 增加推送成功后脏字段、模板和日志路径回归覆盖。

### 密钥与传输参数

- 脱敏改为递归收集配置，覆盖多应用通道中的嵌套 `appToken`、URL 路径 token、分隔 key 和环境变量 JSON 配置。
- 覆盖通道日志、reject、聚合错误、告警和 profile 输出。
- Bark/PushMe 的 `AbortSignal` 仅进入传输选项，不再扩散到第三方 JSON body。

### 失败与缓存事务

- 聚合失败先递归子错误；顶层 permanent 摘要不能覆盖嵌套 retryable 子错误。
- 单次入口和常驻入口对全部推送失败使用同一失败分类；`npm start` 全失败设置非零退出码，部分成功保持成功。
- 存储层区分文件缺失、读取失败、不安全目标和正常读取；缓存重置或原子写入失败时不更新空内存快照。

### 日期与验证

- 无时区的单数字月日日期时间显式按 UTC 解析，不再依赖宿主本地时区。
- 补充 URL/身份、嵌套脱敏、AbortSignal body 隔离、混合失败递归、缓存读取/重置故障注入、跨时区日期和单次退出语义测试。
...
- 异常 getter 字段统一安全读取：模板、HTML 转换、推送构造和缓存写回不再因字段 getter 抛错而中断；已成功推送的消息不会因 `_f` 清理异常改判为失败。
