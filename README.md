...
定时拉取线报酷接口数据 → 规则过滤 → 多通道推送的 Node.js 脚本。零第三方依赖（自制精简 HTTP 模块），641 个测试全绿。
...
三套件分工：`test_filter.js`（单元 568）→ `test_app.js`（集成 57，mock 完整主流程）→ `test_notify.js`（通道 16）。**101 章会自动校验文件头版本号 / CHANGELOG / package.json 三者一致**。
