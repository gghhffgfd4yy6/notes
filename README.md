...
**性能诊断**：`XBK_PROFILE=1` 输出接口/推送/总计；`XBK_PROFILE=2` 输出 DNS 预热、预处理、缓存写入、收尾等待和每个 WxPusher 请求的 wait/dns/tcp/tls/request/firstByte/download 分阶段耗时（不改变默认运行行为）。`XBK_DNS_FAMILY=4/6` 可强制对比 IPv4/IPv6 路径，默认自动。启动时会并行预热 WxPusher DNS，与线报接口请求重叠，减少冷启动推送等待。
