...
> 版本演进见 [CHANGELOG.md](CHANGELOG.md)；当前版本以 [package.json](package.json) 为准（101 章自动校验三方一致，不会过时）
...
三套件分工：`test_filter.js`（单元测试，含 Fuzz 随机冒烟 + 10000 条性能基准）→ `test_app.js`（集成测试，mock 完整主流程）→ `test_notify.js`（通道测试）。测试数量不在此维护（以 `node run_tests.js` 实际输出为准）。**101 章自动校验文件头/CHANGELOG/package.json 三方一致**（README 不含版本号，无需同步）。
