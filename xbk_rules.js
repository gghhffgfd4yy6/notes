      ...
      for (let i = 0; i < s.length; i++) {
        ...
        if (ch === '[') {
          ...
          if (s[j] === '^') {
            j++
            if (s[j] === ']') {
              j++; if (s[j] === ']') j++ // [^]]：[^] 已结束于第二个 ]；否则类结束于此处
            } else {
            // RULES-01：取反类 [^...] 的类体同样要扫描到未转义的类结束符。此前只判 ^ 后的第一个
            // 字符，类体被当作普通模式解析，同一根因造成双向错判：
            //   误拦：'[^(a+)+]x' 的 (a+)+ 被当分组 → 判为嵌套量词（该类其实整体是字符类）；
            //   漏检：'(x[^)]+)+' 的 ) 被吞进类体 → 分组永不闭合 → 真正的嵌套量词漏判。
            // `[^]`（^ 后紧跟 ]）是特例，已在上面的分支按「非空字符类」处理，不走本扫描。
