...
module.exports = {
  ...
  //   跑整套 32 个套件」；换 TAP runner 才能拿到 perTest 覆盖信息（实测快 34–37×；**PR-1 已把 CI 切到 TAP 档**，本地仍走 command 档）。届时按
  ...
  //   要么等后续 PR 退役增量链路（**PR-1 已把 CI 切到 TAP 档**，真全量只需几分钟，强制全量已可负担；本轮按保守路径原样保留复用链路）。
