...
> 文中早期条目出现的 `node_modules/got/index.js`、"自制 got" 均为历史实现记录；当前代码已改用官方 `got@11.8.6`。
> 本文是历史 Bug 记录，不维护当前测试数量、运行耗时或代码行号；当前入口、测试结果和维护阶段状态以项目代码、`package.json`、`CHANGELOG.md` 与 `npm test` 实际输出为准。
...
- **影响**：旧客户端适配层只接受 object；数字/布尔值会被误判为非 JSON，甚至触发 `.slice()` 类型异常。
- **修复**：改用官方 got 后，除字符串响应外，其余已解析 JSON 原始值直接返回。
...
- **历史问题已持续收敛**：文中各项均已在对应版本修复；当前版本和测试结果以 `package.json`、`CHANGELOG.md` 与 `npm test` 实际输出为准。
