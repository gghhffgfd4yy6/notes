...
> **8 项已修复（v3.162，2026-08-03）**——下方每项标注状态、修复方式与验证结果。
...
## 状态说明
- 八个候选均**真实验证触发**、风险明确、收益中-高、修复难度低-中
- **v3.161 已修复 7 项**（2026-08-03），修复顺序：1（wxpusher 当前唯一通道，易触发）→ 2（配置变更体验，缓存语义）→ 3（配置无效无提示）→ 4（格式统一）→ 5（模板配置提示）→ 6（7 通道 API 业务失败静默 → 消息丢失）→ 7（filterHash 漏 pingbitime，改宽不重推）
- **#8 未修复**（2026-08-03 发现，待处理）：v3.158 漏 timeout（第 8 处字符串配置，超时 5s 变 15s + 警告误导）——「修复不完整」型遗漏，触发不难、收益真实
- 测试：test_app +3（t67 过滤变更/pingbitime 警告/占位符警告）+ t68（pingbitime 变更重推）、test_notify +4（wxpusher HTML/autolink + v3.160 息知失败/全通道失败）——**769 全绿**
