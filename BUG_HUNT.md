...
> 本文记录已真实验证的 Bug；当前已记录的问题均已修复，最新修复基线为 **v3.192**。下方每项标注状态、修复方式与验证结果。
...
- **修复**：实体解码后统一调用 `sanitizeDecodedHtml()`，清理 script/style/iframe/object/embed/svg/math、事件属性、危险 URL 以及 xlink/formaction/srcset/style 等加载路径，并覆盖带引号与无引号属性。
