...
const v3tail = allSurvivors.filter(m => m.file === 'xbk_function_v3.js' && m.location.start.line >= 2701)
...
for (const m of v3tail) {
  const s = Math.floor(m.location.start.line / 100) * 100
  seg[s] = (seg[s] || 0) + 1
}
...
for (const m of v3tail.slice(0, 40)) {
  console.log(`  行${m.location.start.line}  ${m.mutatorName.padEnd(26)} 替换→${m.replacement || ''}`)
}
