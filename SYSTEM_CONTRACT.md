...
> 定稿于 v3.179（判重缓存索引化完成、多轮系统审查/验证收敛后）
> 本文件回答「**为什么这么写**」——记录设计理念、系统不变量、模块契约与设计边界。
> 代码可以演进，**契约与理由不随版本过时**；行号标注当前版本位置，以代码为准。
...
- 所有修复必须有测试锁定（项目实践：每个修复配回归测试，变异验证 9/9 检测）。
...
| I2 | **失败的数据不得写入缓存** | pushOne 失败→不进 pushedKeys→toCache 排除（L1821） | 属性测试 500 轮验证 |
...
| I7 | **统计必须自洽** | total = dedup + filtered + truncated + 新推 + 失败 + 坏元素 | 1000 轮稳定性验证 |
| I8 | **判重结果与查询方式无关** | 索引化 ≡ 逐条 has()（属性测试 1800 轮 0 失配） | Set 存在性 ≡ findIndex≥0 |
...
- **为什么索引化**：逐条 `has()` 是 O(N×M)（每条扫全缓存），接口异常返回海量时判重卡死（实测 N=2万/M=1万 → 11.6s）；索引化 O(N+M)（10 万条 → 94ms）。等价性由属性测试证明（800+1000 轮 0 失配）。
...
- 哈希覆盖 `FILTER_FIELDS`（10 个过滤字段）+ pingbitime + zkt_gjc（L275-289）。
...
- **关键契约测试**（改代码必跑）：版本一致性三方（文件头/CHANGELOG/package.json）；判重口径一致性；缓存不变量；ReDoS 全入口；快照（htmlToMarkdown/tuisong_replace）；性能基准（85 章）。
...
7. **行号随版本演进**——本文档行号以 v3.179 为准，结构变化后以代码为准，但契约文字不变。
