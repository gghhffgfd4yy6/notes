...
- 线报接口的 DNS 预热不接取消信号；缓存/pending key 已收敛为 `hostname|family`（不再把 `hints`/`all`/`verbatim` 编进 key），预热与真实请求在默认 auto（family 0）下共用同一条目，显式 `XBK_DNS_FAMILY=4|6` 时预热按 family 4/6 记账、只有真实请求同为该 family 才命中同一条目；`readMessages` 返回内部权威数组的同一引用，调用方必须视为只读。
