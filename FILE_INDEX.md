...
**注意**:文件头版本号(v3.77)需人工维护,但已有 101 章版本一致性测试自动校验(文件头/CHANGELOG 顶部/package.json 三方一致);`require.main === module` 时才自动运行(被 require 时不跑)。
...
版本演进记录(v3.0 → v3.77),每轮修复/重构/功能变更的摘要。
