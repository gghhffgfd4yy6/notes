...
- 本地钩子：`npm run hooks:install` 注册 `.githooks`（`pre-commit` 跑 lint / 版本闸门 / `npm run test:filter`；`commit-msg` 要求首行以 `fix: feat: refactor: docs: chore: style: test: perf: revert: build: ci:` 之一开头且不超过 100 字符）。
