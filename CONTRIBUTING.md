...
- 本地钩子：`npm run hooks:install` 注册 `.githooks`（`pre-commit` 跑 lint / 版本闸门 / 变异行段校验（约 20s），`pre-push` 跑 `npm run test:filter`（约 60s，推送前拦截）；`commit-msg` 要求首行以 `fix: feat: refactor: docs: chore: style: test: perf: revert: build: ci:` 之一开头且不超过 100 字符）。npm 不会自动注册仓库钩子，克隆后**必须显式装一次**；装完跑 `npm run hooks:verify` 确认门禁真的生效（只读，未生效 exit 1）。
