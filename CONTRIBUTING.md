...
- 变异**分数门禁**（CI）：`stryker.config.js` 的 `thresholds.break = 65`，CI 上按**段**判定——某段 `mutationScore` 跌破 65，该矩阵 job 红（其余段照跑，`report` job 仍发日报）。本地 `npm run test:mutation` 用同一阈值、但按**全量**分数判定（合计基线 81.27%），因此本地实际只会在整体跌到 65 以下时才非 0 退出。阈值低于实测最低段（`message-store` 69.38%），CI 上某段真实退化**超过 4.38 个百分点**（分数严格跌破 65 才红：69.38→65.00 不红、→64.99 才红）即会拦下；回退＝把 `break` 改回 `null`。日报里的合计/分段分数是观察基线，不是门禁口径。
